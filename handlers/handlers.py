...
async def help_handler(msg: Message, state: FSMContext):
    try:
        ...
        await msg.answer("""Инструкция по использованию бота для поиска вакансий

Начало работы: Для начала взаимодействия с ботом отправьте команду /start. Бот приветствует вас и предлагает помощь в поиске вакансий.
Получение помощи: Если вам нужна дополнительная информация о том, как использовать бота, отправьте команду /help. В ответ вы получите краткое руководство по основным функциям бота.
Поиск вакансий: Чтобы начать поиск вакансий, отправьте команду /update_data. Затем введите ключевые слова для поиска вакансий. После ввода ключевых слов бот запросит, хотите ли вы добавить фильтры поиска (например, зарплата, опыт работы).
Добавление фильтров поиска: Ответьте на вопрос бота о добавлении фильтров. Если вы согласны, укажите необходимые параметры (например, диапазон зарплаты, уровень опыта). Это поможет уточнить результаты поиска.
Выбор количества вакансий: Укажите количество вакансий, которые вы хотите просмотреть. Это поможет ограничить объем информации, который будет передан вам.
Указание поля поиска: Выберите, по какому полю следует искать вакансии (название вакансии, название компании, описание вакансии). Можно указать несколько полей, разделив их пробелами.
Выбор опыта работы: Укажите ваш предпочтительный уровень опыта работы для найденных вакансий. Это может быть "нет опыта", "от 1 до 3 лет", "от 3 до 6 лет" или "более 6 лет".
Сортировка результатов: Выберите критерии сортировки результатов поиска (по времени публикации, по убыванию зарплаты, по возрастанию зарплаты или по степени соответствия ключевым словам).
После выполнения всех этих шагов бот соберет и подготовит список вакансий, соответствующих вашим критериям, и отправит его вам.""")
